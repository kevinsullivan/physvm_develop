...
std::string Interp::toString(){
    ...
    if (nodeType =="COMPOUND_STMT") {
        ...
        retval += std::string("def ") + (vardom_->hasValue() ? vardom_->getValue()->getName() : "") + " : " + (var_->getType()) + " := " + expr_->toString();//[(" +  + ")]";
    }
        ...
        retval += (lhs_->hasValue()?lhs_->toString():"_") + "+ᵥ" + (rhs_->hasValue()?rhs_->toString():"_");
    }
