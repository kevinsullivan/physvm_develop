...
def derived_fr := affine_coord_space.mk_frame 
    sp1 
    pt1' 
    (affine_coord_space.mk_basis sp1 ⟨[vec1'], sorry⟩)
    sorry
...
/-
Note that the class command, as discussed in Section 4.10, does more than simply assign 
the attribute. There are attributes that control how eagerly definitions are unfolded 
during elaboration:
...
-- example : base_sp = sp1 := eq.refl sp1
...
-/
/-
frame #1 type == (affine_coord_frame.standard ℝ 1)

frame #2 type == 
(affine_coord_frame.base_frame
          (aff_fr.affine_coord_frame.derived 
            (affine_coord_frame.get_coords derived_fr).origin
             (λ (i : fin 1), (affine_coord_frame.get_coords derived_fr).basis i)
             derived_sp._proof_1
             (affine_coord_frame.standard ℝ 1))
)
{b : frame}, E b : newframe
a : frame
b =/= a
E b = E a
-/
...
example :   (affine_coord_frame.standard ℝ 1) = 
            (affine_coord_frame.base_frame
                (aff_fr.affine_coord_frame.derived 
                (affine_coord_frame.get_coords derived_fr).origin
                (λ (i : fin 1), (affine_coord_frame.get_coords derived_fr).basis i)
