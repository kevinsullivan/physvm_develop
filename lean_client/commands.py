...
class OkResponse(Response):
    ...
    def to_command_response(self, command: str) -> CommandResponse:
        response_types: List[Type[CommandResponse]] = [
            ...
            SearchResponse, AllHoleCommandsResponse, HoleResponse, RoiResponse, GetWidgetResponse
        ]
...
class InfoRequest(Request):
    ...
    column: int
    def to_json(self) -> str:
        return '{"file_name": "'+self.file_name+'", "line": ' \
            + str(self.line) + ', "column": '+ \
                str(self.column) + ', "seq_num": '+ str(self.line) + ', "command": "info"}'
#{"file_name": "/peirce/PeirceOutput.lean", "line": "13", "column": "1", "seq_num": 107, "command": "info"}
...
class InfoRecord:
    ...
    widget: Optional[WidgetIdentifier] = None
...
'''
@dataclass
class GetWidgetRequest(Request):
    expect_response = True
    command=  'get_widget'
    file_name: str
    line: int
...
class WidgetElement:
    t: Optional[str] = None
    c: Optional[List[WidgetComponent]] = None
    a: Optional[str] = None
    e: Optional[str] = None
    tt: Optional[WidgetComponent] = None
...
@dataclass
class GetWidgetResponse(CommandResponse):
...
class RoiRequest(Request):
    ...
    def to_json(self) -> str:
        dic = self.__dict__.copy()
        dic['command'] = 'roi'
        dic['mode'] = 'visible-lines'#dic['mode'].name
