...
-/
def env1 : @env.env K _ _ (std_frame K) (std_space K) := env.env.init K

--Bind a time object to variable and in environment
def assign_t1 :=
  cmd.assign_time ⟨⟨"t1"⟩⟩ (lang.time.time_expr.lit (mk_time (std_space K) 5 ))
def env2 := cmd_eval K env1 assign_t1

--Bind a time object to variable and in environment
def assign_t2 :=
  cmd.assign_time ⟨⟨"t2"⟩⟩ (lang.time.time_expr.var (⟨⟨"t1"⟩⟩ : lang.time.time_var (std_space K)))
def env3 := cmd_eval K env2 assign_t2

--When it is computable, we have access to the coordinates just fine.
#eval (lang.time_eval.time_eval (std_space K) env3.t (lang.time.time_expr.var (⟨⟨"t1"⟩⟩ : lang.time.time_var (std_space K))))
  .to_point.to_pt.to_prod

--Bind a duration object by subtracting times (which are evaluated from variables)
def assign_d1 :=
  let eval_t1 := (lang.time_eval.time_eval (std_space K) env3.t (lang.time.time_expr.var (⟨⟨"t1"⟩⟩ : lang.time.time_var (std_space K)))) in
  let eval_t2 := (lang.time_eval.time_eval (std_space K) env3.t (lang.time.time_expr.var (⟨⟨"t2"⟩⟩ : lang.time.time_var (std_space K)))) in
...
def assign_robot_frame :=
  let eval_t1 := (lang.time_eval.time_eval (std_space K) env3.t (lang.time.time_expr.var (⟨⟨"t1"⟩⟩ : lang.time.time_var (std_space K)))) in
  let eval_d1 := (lang.time_eval.duration_eval (std_space K) env3.t (lang.time.duration_expr.var (⟨⟨"d1"⟩⟩ : lang.time.duration_var (std_space K)))) in
  --starts getting ugly - cmd needs some explicit arguments, which hardly make sense
  --@cmd.assign_fm K _ _ (std_frame K) (std_space K)
  @cmd.assign_fm K _ _ (std_frame K) (std_space K) ⟨⟨"robot"⟩⟩ (lang.math.fm_expr.lit (mk_frame eval_t1.to_point eval_d1.to_vectr))

def env5 := cmd_eval K env4 assign_robot_frame

def assign_robot_space :=
  let eval_robot := (lang.math_eval.fm_eval (std_frame K)/-<-what is this doing here-/ 
    env4.m (lang.math.fm_expr.var (⟨⟨"robot"⟩⟩))) in
  --@cmd.assign_spc K _ _ (std_frame K) (std_space K) ⟨⟨"robot"⟩⟩ (mk_space K eval_robot) 
  --odd but functional.
