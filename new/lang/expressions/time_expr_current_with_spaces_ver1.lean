...
/-Time expressions and Duration expressions cannot depend on Frame Expressions or Space Expressions
because literal expressions require a phys Time or Duration object, which depend on fm and spc. Again,
we need time and duration expressions to depend on fm and spc if we require type checking of expressions like
(d1 : time_expr sp1) +ᵥ (d2 : time_expr sp2) at the lang level.
 Thus, there is no reason to define frame_expr or space_expr antecedent to time_expr or duration_expr, although
aesthetically we may prefer that a lang point expressions depends on a frame expression rather than a literal value
at a lower level.

Instead, we need the definition of frame expression to follow the definition of point and vector expressions,
as we might be interested in constructing new time frames from existing point and vector expressions. 

As the definition of spc follows the definition of fm and depends on fm, we follow the same convention here. 
