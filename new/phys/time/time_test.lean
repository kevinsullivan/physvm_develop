...
noncomputable def t1 : time (std_space K TIME) := mk_time (std_space K TIME)  1          -- t=0 + 1 second
noncomputable def t2 := mk_time (std_space K TIME) 3                             -- t=0 + 3 seconds
noncomputable def d1 : duration (std_space K TIME) := t2 -ᵥ t1                     -- 2 seconds
noncomputable def t3 := 5 • d1 +ᵥ t1                                          -- t=0 + 11 seconds

-- new frame with origin at t=0 + 1hr and unit duration of 1 minute
noncomputable def t1' := mk_time (std_space K TIME) 3600      -- t=0 + 1hr
noncomputable def d1' := 60 • (mk_duration (std_space K TIME) 1)                                             -- one minute
