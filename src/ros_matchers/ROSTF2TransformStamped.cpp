...
void ROSTF2TransformStamped::setup(){
...
void ROSTF2TransformStamped::run(const MatchFinder::MatchResult &Result){
    ...
    if(cxxConstructExpr_){
        ...
        {
            ROSTF2TransformStamped pm{context_, interp_};
            pm.setup();
	...
	arg_decay_exist_predicates["callExpr_tf2::Stamped<tf2::Transform>"] = [=](std::string typenm){
    ...
    if(memberExpr_){
        ...
        auto typestr = this->getTypeAsString(inner,false);
        if(false){}
    ...
    {
        ROSTF2TransformStamped exprMatcher{ context_, interp_};
        exprMatcher.setup();
    ...
    if (parenExpr_)
    {
        ROSTF2TransformStamped inner{ context_, interp_};
        inner.setup();
    ...
    if (cxxOperatorCallExpr_){
        ...
        if(auto dc = clang::dyn_cast<clang::NamedDecl>(decl_)){
            ...
            if(name == "operator="){
                ...
                ROSTF2TransformStamped rhsMatcher{ context_, interp_};
                rhsMatcher.setup();
                ...
                if(hasmemb){
                    ...
                    auto typestr = this->getTypeAsString(lhs,false);
