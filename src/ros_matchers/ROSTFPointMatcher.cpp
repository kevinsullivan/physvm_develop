...
*/
    if(parenExpr_){
        ...
        if(decl_->isCopyOrMoveConstructor())
        {
            ...
            a2.search();
            a3.search();
            a1.visit(*cxxConstructExpr_->getArg(0));
            a2.visit(*cxxConstructExpr_->getArg(1));
            a3.visit(*cxxConstructExpr_->getArg(2));
        ...
        if(decl_->isCopyOrMoveConstructor())
        {
            ROSTFPointMatcher pm{context_, interp_};
            pm.search();
            pm.visit(**cxxTemporaryObjectExpr_->getArgs());
            this->childExprStore_ = pm.getChildExprStore();
        }
            ...
            a2.search();
            a3.search();
            a1.visit(*cxxTemporaryObjectExpr_->getArg(0));
            a2.visit(*cxxTemporaryObjectExpr_->getArg(1));
            a3.visit(*cxxTemporaryObjectExpr_->getArg(2));
            interp_->mkREAL3_LIT_REAL1_EXPR_REAL1_EXPR_REAL1_EXPR(
