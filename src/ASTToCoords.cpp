...
void ASTToCoords::overrideExpr(const clang::Expr *e, const coords::VectorExpr *c) {
    expr_coords.insert(std::make_pair(e, c));
}

void ASTToCoords::overrideStmt(const clang::Stmt *s, const coords::VectorExpr *c) {
    stmt_coords.insert(std::make_pair(s, c));
}

void ASTToCoords::overrideDecl(const clang::Decl *d, const coords::VectorExpr *c) {
    cerr << "ASTToCoords::overrideDecl: AST " << std::hex << d << "to coords " 
        << std::hex << c << " toString " << c->toString() << "\n";
    decl_coords.insert(std::make_pair(d, c));
}

const coords::VectorExpr *ASTToCoords::getASTExprCoords(ast::Expr* e) {
