...
Name - Derived Frame Match
Description - 
    The purpose of this test is to demonstrate that vector and point operations 
    using the same derived frame do not trigger an error.
Expected Outcome - 
    Several spaces are created. An alias for a standard frame is created.
    A derived frame is created from the standard frame.
    Two points are created, respectively expressed in the derived frames.
    Those points are subtracted into a vector, which  does not trigger an error.
Implementation Gaps - 
    Same gaps as Test 4.
  -Layer 1 (Parsers) : 
  -Layer 2 (Peirce) :
...
int main(int argc, char **argv){
    ...
    //8 : @@EuclideanGeometryPoint tf_start_point = EuclideanGeometryPoint(worldGeometry,Value=<10,10,10>,robotFrame,si)
    tf::Point
    ...
    //10 (INFERRED?) : @@EuclideanGeometryVector tf_displacement = EuclideanGeometryVector(worldGeometry,Value=<-10,12,-2>,robotFrame,si)
