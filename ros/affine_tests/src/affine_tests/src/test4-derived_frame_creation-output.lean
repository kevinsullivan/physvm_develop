...
int main(int argc, char **argv){
    ...
    //7 : @@EuclideanGeometryPoint tf_start_point = EuclideanGeometryPoint(worldGeometry,Value=<10,10,10>,stdWorldFrame)
    tf::Point
        tf_start_point = tf::Point(10, 10, 10);
    
    //8 : @@EuclideanGeometryPoint tf_end_point = EuclideanGeometryPoint(worldGeometry,Value=<20,-2,12>,robotFrame)
    tf::Point
        tf_end_point = tf::Point(20, -2, 12);

    //9 (Error?) : @@EuclideanGeometryVector tf_displacement = EuclideanGeometryVector(worldGeometry,Value=<-10,12,-2>,worldGeometry.stdFrame)
