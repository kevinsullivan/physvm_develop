...
void lookup_and_attempt_transform(geometry_msgs::Vector3Stamped v, const tf2_ros::Buffer& tfb, std::string inframe, std::string outframe){
    try{
        ...
        transform = tfb.lookupTransform(inframe, outframe, ros::Time(0));   /* TODO: DOCUMENT BROKEN API SPEC? API Docs are backwards, out/in, not in/out */
        geometry_msgs::Vector3Stamped new_vec;                              // return result storage
        tf2::doTransform(v, new_vec, transform);                            // compute the result of applying the transform
                                                                            // lo and behold no error checking here -- no exception raised if wrong args
...
void test_incorrect_ordering(const tf2_ros::Buffer& tfb){
    ...
    p.pose.position.y = 1;
    p.pose.position.z = 1;
    p.header.frame_id = "two";
    tf::quaternionTFToMsg(qone, p.pose.orientation);

    geometry_msgs::PoseStamped new_pt;
    tf2::doTransform(p, new_pt, transform);
    ROS_INFO_STREAM(p);
    ROS_INFO_STREAM(transform);
    ROS_INFO_STREAM(new_pt);
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "physical_inconsistencies_main");
    ros::NodeHandle node;
